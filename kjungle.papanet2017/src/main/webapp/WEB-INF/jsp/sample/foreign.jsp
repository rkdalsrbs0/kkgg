...
<div class="container">
		<div class="lnb">
			...
			<div class="lnb_menu">
				<a href="/sample/foreignList/1" class="on">해외 아빠육아 정책사례</a>
		...
		<c:if test="${display eq 'normal'}">
			<div class="paginate">
				<a href="/sample/foreignList/1" class="pre"><</a>
				<c:if test="${pageno!=1}">
					<a href="/sample/foreignList/${prev_pageno}">Previous</a>
				</c:if>
				
				<c:forEach var="i" begin="1" end="${page_eno}" step="1">
					<a href="/sample/foreignList/${i}">
						<c:choose>
				...
				<c:if test="${pageno<total_page}">
					<a href="/sample/foreignList/${pageno+1 }">Next</a>
				</c:if>
				<a href="/sample/foreignList/${total_page }" class="next">></a>
			</div>
		</c:if>
		
		<c:if test="${display eq 'search'}">
			<div class="paginate">
				<a href="/sample/foreignList/${search }/1" class="pre"><</a>
				<c:if test="${pageno!=1}">
					<a href="/sample/foreignList/${search }/${prev_pageno}">Previous</a>
				</c:if>
				<c:forEach var="i" begin="1" end="${page_eno}" step="1">
					<a href="/sample/foreignList/${search }/${i}">
						<c:choose>
							<c:when test="${pageno==i}">
								<strong>${i}</strong>
							</c:when>
				...
				<c:if test="${pageno<total_page}">
					<a href="/sample/foreignList/${search }/${pageno+1 }">Next</a>
				</c:if>
				<a href="/sample/foreignList/${search }/${total_page }" class="next">></a>
			</div>
		</c:if>
		</div>
	</div>
...
function searchEnterkey(){
	 if (window.event.keyCode == 13) {
		frmObj.action="/sample/foreign/search"
		frmObj.submit();
	 }
 }
function search(){
