...
<div class="container">
		...
		<c:if test="${fn:length(workList) != 0}">
			<div class="paginate">
				<a href="/essay/1"><</a>
				<a href="/essay/${prev_pageno}">Previous</a>
				...
				<c:if test="${pageno<total_page}">
					<a href="/essay/${pageno+1 }">Next</a>
				</c:if>
				<a href="/essay/${total_page }">></a>
			</div>
		</c:if>
		
		<c:if test="${fn:length(searchList) != 0}">
			<div class="paginate">
				<a href="/essay/${search}/1"><</a>
				<a href="/essay/${search}/${prev_pageno}">Previous</a>
				<c:forEach var="i" begin="${page_sno}" end="${page_eno}" step="1">
					<a href="/essay/${search }/${i}">
						<c:choose>
							<c:when test="${pageno==i}">
								<strong>${i}</strong>
							</c:when>
				...
				<c:if test="${pageno<total_page}">
					<a href="/essay/${search }/${pageno+1 }">Next</a>
				</c:if>
				<a href="/essay/${search }/${total_page }">></a>
			</div>
		</c:if>
		-->
	</div>
...
$(document).ready(function(){
	...
	var history = sessionStorage.getItem('scrollEssay');
	$(this).scrollTop(history);
});

$(window).on("beforeunload", function(){
	var scrlNum = $(this).scrollTop();
	sessionStorage.setItem('scrollEssay', scrlNum);
});
...
var frmObj = document.work_View;
	...
	function view(idx, valuer){
		...
		frmObj.action ="/essay/view";
		frmObj.submit();
	}
	
	function search(){
		frmObj.action="/essay/search"
		frmObj.submit();
	}
</script>