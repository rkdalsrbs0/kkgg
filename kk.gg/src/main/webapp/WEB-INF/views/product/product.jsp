...
<div class="container">
		...
		<c:if test="${fn:length(workList) != 0}">
			<div class="paginate">
				<a href="/product/1"><</a>
				<a href="/product/${prev_pageno}">Previous</a>
				...
				<c:if test="${pageno<total_page}">
					<a href="/product/${pageno+1 }">Next</a>
				</c:if>
				<a href="/product/${total_page }">></a>
			</div>
		</c:if>
		
		<c:if test="${fn:length(searchList) != 0}">
			<div class="paginate">
				<a href="/product/${search}/1"><</a>
				<a href="/product/${search}/${prev_pageno}">Previous</a>
				<c:forEach var="i" begin="${page_sno}" end="${page_eno}" step="1">
					<a href="/product/${search }/${i}">
						<c:choose>
							<c:when test="${pageno==i}">
								<strong>${i}</strong>
							</c:when>
				...
				<c:if test="${pageno<total_page}">
					<a href="/product/${search }/${pageno+1 }">Next</a>
				</c:if>
				<a href="/product/${search }/${total_page }">></a>
			</div>
		</c:if>
		 -->
	</div>
...
$(document).ready(function(){
	...
	var history = sessionStorage.getItem('scrollIdea');
	$(this).scrollTop(history);
});

$(window).on("beforeunload", function(){
	var scrlNum = $(this).scrollTop();
	sessionStorage.setItem('scrollIdea', scrlNum);
});
...
var frmObj = document.work_View;
	...
	function view(idx, valuer){
		...
		frmObj.action ="/product/view";
		frmObj.submit();
	}
	
	function search(){
		frmObj.action="/product/search"
		frmObj.submit();
	}
</script>